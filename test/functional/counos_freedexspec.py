...
class CounosFreeDExSpec(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        txid = self.nodes[0].counos_sendactivation(address, 15, activation_block, 0)
        self.nodes[0].generatetoaddress(1, coinbase_address)
        ...
        featureid = self.nodes[0].counos_getactivations()['completedactivations']
        freeDEx = False
        ...
        txid = self.nodes[0].counos_sendissuancefixed(address, 1, 2, 0, "Test Category", "Test Subcategory", "TST", "http://www.counoslayer.org", "This is a test for managed properties", "100000")
        self.nodes[0].generatetoaddress(1, coinbase_address)
        ...
        activeOffersAtTheStart = self.nodes[0].counos_getactivedexsells()
        startBTC = 0.1
        startMSC = "2.5"
        ...
        txid = self.nodes[0].counos_send(address, fundedAddress, currencyOffered, startMSC)
        self.nodes[0].generatetoaddress(1, coinbase_address)
        ...
        txid = self.nodes[0].counos_senddexsell(fundedAddress, currencyOffered, amountOffered, desiredBTC, stdBlockSpan, stdCommitFee, actionNew)
        self.nodes[0].generatetoaddress(1, coinbase_address)
        ...
        offerTx = self.nodes[0].counos_gettransaction(txid)
        ...
        assert_equal(len(self.nodes[0].counos_getactivedexsells()), len(activeOffersAtTheStart) + 1)
        ...
        # Setup variables
        amountAvailableAtStart = self.nodes[0].counos_getbalance(fundedAddress, currencyOffered)['balance']
        ...
        payload = self.nodes[0].counos_createpayload_dexsell(currencyOffered, str(amountOffered), desiredBTC, stdBlockSpan, stdCommitFee, actionNew)
        offerTxid = self.nodes[0].counos_sendrawtx(fundedAddress, payload)
        self.nodes[0].generatetoaddress(1, coinbase_address)

        # Checking the transaction was valid...
        offerTx = self.nodes[0].counos_gettransaction(offerTxid)
        offerAmount = offerTx['amount']
        amountAvailableNow = self.nodes[0].counos_getbalance(fundedAddress, currencyOffered)['balance']
        assert_equal(offerTx['valid'], True)
        ...
        balanceAtStart = self.nodes[0].counos_getbalance(fundedAddress, currencyOffered)
        ...
        offerTx = self.nodes[0].counos_gettransaction(txid)
        offerAmount = offerTx['amount']
        balanceNow = self.nodes[0].counos_getbalance(fundedAddress, currencyOffered)
        ...
        payload = self.nodes[0].counos_createpayload_dexsell(currencyOffered, offerMSC, desiredBTC, stdBlockSpan, stdCommitFee, actionNew)
        offerTxid = self.nodes[0].counos_sendrawtx(fundedAddress, payload)
        self.nodes[0].generatetoaddress(1, coinbase_address)

        # Checking the transaction was valid...
        offerBeforeReceivingMore = self.nodes[0].counos_gettransaction(offerTxid)
        assert_equal(offerBeforeReceivingMore['valid'], True)

        # Store balance now before receiving more
        balanceBeforeReceivingMore = self.nodes[0].counos_getbalance(fundedAddress, currencyOffered)
        ...
        txid = self.nodes[0].counos_send(address, otherAddress, currencyOffered, startOtherMSC)
        self.nodes[0].generatetoaddress(1, coinbase_address)
        ...
        sendTxid = self.nodes[0].counos_send(otherAddress, fundedAddress, currencyOffered, additionalMSC)
        self.nodes[0].generatetoaddress(1, coinbase_address)

        # Checking the transaction was valid...
        sendTx = self.nodes[0].counos_gettransaction(sendTxid)
        ...
        offerNow = self.nodes[0].counos_gettransaction(offerTxid)
        ...
        txid = self.nodes[0].counos_senddexsell(fundedAddress, currencyOffered, firstOfferMSC, firstOfferBTC, stdBlockSpan, stdCommitFee, actionNew)
        self.nodes[0].generatetoaddress(1, coinbase_address)
        ...
        payload = self.nodes[0].counos_createpayload_dexsell(currencyOffered, secondOfferMSC, secondOfferBTC, stdBlockSpan, stdCommitFee, actionNew)
        txid = self.nodes[0].counos_sendrawtx(fundedAddress, payload)
        self.nodes[0].generatetoaddress(1, coinbase_address)
        ...
        # Get offers
        offersAtStart = self.nodes[0].counos_getactivedexsells()

        # creating an offer with action 1
        txid = self.nodes[0].counos_senddexsell(fundedAddress, currencyOffered, offeredMSC, desiredBTC, stdBlockSpan, stdCommitFee, actionNew)
        self.nodes[0].generatetoaddress(1, coinbase_address)
        ...
        assert_equal(len(self.nodes[0].counos_getactivedexsells()), len(offersAtStart) + 1)

        # updating an offer with action = 2
        payload = self.nodes[0].counos_createpayload_dexsell(currencyOffered, updatedMSC, updatedBTC, stdBlockSpan, stdCommitFee, actionUpdate)
        txid = self.nodes[0].counos_sendrawtx(fundedAddress, payload)
        self.nodes[0].generatetoaddress(1, coinbase_address)
        ...
        txid = self.nodes[0].counos_senddexsell(fundedAddress, currencyOffered, "0", "0", 0, "0", actionCancel)
        self.nodes[0].generatetoaddress(1, coinbase_address)
        ...
        assert_equal(len(self.nodes[0].counos_getactivedexsells()), len(offersAtStart))
        ...
        txid = self.nodes[0].counos_send(address, actorA, currencyOffered, startMSC)
        self.nodes[0].generatetoaddress(1, coinbase_address)
        ...
        # A offers MSC
        txid = self.nodes[0].counos_senddexsell(actorA, currencyOffered, offeredMSC, desiredBTC, stdBlockSpan, stdCommitFee, actionNew)
        self.nodes[0].generatetoaddress(1, coinbase_address)
        ...
        # B accepts the offer
        txid = self.nodes[0].counos_senddexaccept(actorB, actorA, currencyOffered, offeredMSC, False)
        self.nodes[0].generatetoaddress(1, coinbase_address)
