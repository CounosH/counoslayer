...
class CounosFreeze(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        node.counos_sendissuancemanaged(address, 1, 1, 0, "TestCat", "TestSubCat", "TestProperty", "TestURL", "TestData")
        node.generatetoaddress(1, coinbase_address)
        node.counos_sendgrant(address, freeze_address, 3, "1000")
        node.generatetoaddress(1, coinbase_address)

        # Running the test scenario...
        # Sending a 'freeze' tranasction for the test address prior to enabling freezing
        txid = node.counos_sendfreeze(address, freeze_address, 3, "1234")
        node.generatetoaddress(1, coinbase_address)
        ...
        txid = node.counos_send(freeze_address, address, 3, "50")
        node.generatetoaddress(1, coinbase_address)
        ...
        balance = node.counos_getbalance(freeze_address, 3)['balance']
        assert_equal(balance, "1000")

        # Sending an 'unfreeze' tranasction for the test address
        txid = node.counos_sendunfreeze(address, freeze_address, 3, "1234")
        node.generatetoaddress(1, coinbase_address)
        ...
        txid = node.counos_senddisablefreezing(address, 3)
        node.generatetoaddress(1, coinbase_address)
        ...
        txid = node.counos_send(freeze_address, address, 3, "30")
        node.generatetoaddress(1, coinbase_address)
        ...
        txid = node.counos_sendactivation(address, 14, blockcount, 999)
        node.generatetoaddress(1, coinbase_address)
        ...
        txid = node.counos_sendall(freeze_address, address, 1)
        node.generatetoaddress(1, coinbase_address)
