...
class CounosReorgTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Create test token
        creation_tx = self.nodes[0].counos_sendissuancefixed(token_address, 2, 2, 0, "", "", "TESTTOKEN", "", "", "100000")
        self.nodes[0].generatetoaddress(1, token_address)
        self.sync_all()

        # Send token
        property_id = self.nodes[0].counos_gettransaction(creation_tx)["propertyid"]
        destination = self.nodes[1].getnewaddress()
        self.nodes[0].counos_send(token_address, destination, property_id, "1")
        ...
        assert_equal(str(self.nodes[0].counos_getbalance(destination, property_id)["balance"]), "1.00000000")
        assert_equal(str(self.nodes[1].counos_getbalance(destination, property_id)["balance"]), "1.00000000")

if __name__ == '__main__':
    CounosReorgTest().main()
