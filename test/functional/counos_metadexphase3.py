...
class CounosMetaDexPhase3(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Testing a trade against self that uses divisible / divisible (10.0 COUNOS for 100.0 #4)
        txid = self.nodes[0].counos_sendtrade(address, 3, "20", 1, "1.1")
        self.nodes[0].generatetoaddress(1, coinbase_address)
        ...
        # Testing a trade against self that doesn't use Counos to confirm non-Counos pairs are disabled (4.45 #4 for 20 #3)
        txid = self.nodes[0].counos_sendtrade(address, 3, "20", 4, "4.45")
        self.nodes[0].generatetoaddress(1, coinbase_address)
        ...
        # Testing a trade against self that doesn't use Counos to confirm non-Counos pairs are now enabled (4.45 #4 for 20 #3)
        txid = self.nodes[0].counos_sendtrade(address, 3, "20", 4, "4.45")
        self.nodes[0].generatetoaddress(1, coinbase_address)
