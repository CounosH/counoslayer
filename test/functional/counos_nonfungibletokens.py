...
class counosNonFungibleTokensTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Create test token
        txid = self.nodes[0].counos_sendissuancemanaged(token_address, 2, 5, 0, "", "", "TESTTOKEN", "", "")
        self.nodes[0].generatetoaddress(1, token_address)

        # Checking the transaction was valid...
        result = self.nodes[0].counos_gettransaction(txid)
        assert_equal(result['valid'], True)
        property_id = result["propertyid"]

        # Send tokens we do not have yet
        try:
            self.nodes[0].counos_sendnonfungible(token_address, destination_address, property_id, 1, 1)
        ...
        txid = self.nodes[0].counos_sendgrant(token_address, "", property_id, "100", "")
        self.nodes[0].generatetoaddress(1, token_address)

        # Send tokens out of range start
        try:
            self.nodes[0].counos_sendnonfungible(token_address, destination_address, property_id, 0, 10)
        except JSONRPCException as e:
            errorString = e.error['message']
        ...
        try:
            self.nodes[0].counos_sendnonfungible(token_address, destination_address, property_id, 100, 101)
        except JSONRPCException as e:
            errorString = e.error['message']
        assert("Sender does not own the range" in errorString)
        errorString = ""
        ...
        result = self.nodes[0].counos_getproperty(property_id)
        ...
        result = self.nodes[0].counos_getnonfungibletokens(token_address, property_id)
        ...
        result = self.nodes[0].counos_getnonfungibletokenranges(property_id)
        ...
        # Check data blank
        result = self.nodes[0].counos_getnonfungibletokendata(property_id, 1)
        ...
        txid = self.nodes[0].counos_sendgrant(token_address, "", property_id, "1", "Test grantdata")
        self.nodes[0].generatetoaddress(1, token_address)
        ...
        result = self.nodes[0].counos_getnonfungibletokendata(property_id, 101)
        assert_equal(result[0]['owner'], token_address)
        assert_equal(result[0]['grantdata'], 'Test grantdata')

        # Grant 99 tokens with different data
        txid = self.nodes[0].counos_sendgrant(token_address, "", property_id, "99", "Different grantdata")
        self.nodes[0].generatetoaddress(1, token_address)
        ...
        result = self.nodes[0].counos_getnonfungibletokendata(property_id, 200)
        assert_equal(result[0]['owner'], token_address)
        ...
        txid = self.nodes[0].counos_sendgrant(token_address, grant_address, property_id, "100", "Multiple grantdata")
        self.nodes[0].generatetoaddress(1, token_address)
        ...
        result = self.nodes[0].counos_getnonfungibletokens(grant_address, property_id)
        ...
        # Check data fields
        result = self.nodes[0].counos_getnonfungibletokendata(property_id, 300)
        ...
        self.nodes[0].counos_sendnonfungible(token_address, destination_address, property_id, 1, 10)

        # Send single token with "Test grantdata"
        self.nodes[0].counos_sendnonfungible(token_address, destination_address, property_id, 101, 101)

        # Send token range from different address with "Multiple grantdata"
        self.nodes[0].counos_sendnonfungible(grant_address, destination_address, property_id, 201, 210)
        self.nodes[0].generatetoaddress(1, token_address)
        ...
        result = self.nodes[0].counos_getnonfungibletokens(destination_address, property_id)
        assert_equal(result[0]['tokens'][0]['tokenstart'], 1)
        ...
        result = self.nodes[0].counos_getnonfungibletokendata(property_id, 201)
        assert_equal(result[0]['owner'], destination_address)
        assert_equal(result[0]['grantdata'], 'Multiple grantdata')
        assert_equal(result[0]['issuerdata'], '')
        assert_equal(result[0]['holderdata'], '')
        ...
        try:
            self.nodes[1].counos_setnonfungibledata(property_id, 102, 102, True, "Test issuerdata")
        except JSONRPCException as e:
            errorString = e.error['message']
        ...
        try:
            self.nodes[1].counos_setnonfungibledata(property_id, 102, 102, False, "Test holderdata")
        except JSONRPCException as e:
            errorString = e.error['message']
        assert("Error with selected inputs for the send transaction" in errorString)
        errorString = ""
        ...
        self.nodes[0].counos_sendnonfungible(token_address, non_issuer_address, property_id, 102, 111)
        self.nodes[0].generatetoaddress(1, token_address)
        self.sync_all()
        ...
        result = self.nodes[0].counos_getnonfungibletokendata(property_id, 102)
        ...
        # End new data
        result = self.nodes[0].counos_getnonfungibletokendata(property_id, 103)
        assert_equal(result[0]['owner'], non_issuer_address)
        assert_equal(result[0]['grantdata'], 'Different grantdata')
        assert_equal(result[0]['issuerdata'], '')
        assert_equal(result[0]['holderdata'], '')
        ...
        self.nodes[1].counos_setnonfungibledata(property_id, 102, 111, False, "New holderdata")
        self.sync_mempools()
        self.nodes[0].generatetoaddress(1, token_address)
        self.sync_blocks()
        ...
        # End new data
        result = self.nodes[0].counos_getnonfungibletokendata(property_id, 111)
        assert_equal(result[0]['owner'], non_issuer_address)
        assert_equal(result[0]['grantdata'], 'Different grantdata')
        assert_equal(result[0]['issuerdata'], '')
        assert_equal(result[0]['holderdata'], 'New holderdata')
        ...
        # After blank
        result = self.nodes[0].counos_getnonfungibletokendata(property_id, 112)
        assert_equal(result[0]['owner'], token_address)
        assert_equal(result[0]['grantdata'], 'Different grantdata')
        assert_equal(result[0]['issuerdata'], '')
        assert_equal(result[0]['holderdata'], '')
        ...
        self.nodes[1].counos_setnonfungibledata(property_id, 106, 106, False, "Even newer holderdata")
        self.sync_mempools()
        self.nodes[0].generatetoaddress(1, token_address)
        self.sync_blocks()
        ...
        # Before range end
        result = self.nodes[0].counos_getnonfungibletokendata(property_id, 105)
        assert_equal(result[0]['owner'], non_issuer_address)
        assert_equal(result[0]['grantdata'], 'Different grantdata')
        assert_equal(result[0]['issuerdata'], '')
        assert_equal(result[0]['holderdata'], 'New holderdata')
        ...
        # Data changed?
        result = self.nodes[0].counos_getnonfungibletokendata(property_id, 106)
        assert_equal(result[0]['owner'], non_issuer_address)
        assert_equal(result[0]['grantdata'], 'Different grantdata')
        assert_equal(result[0]['issuerdata'], '')
        ...
        # After range
        result = self.nodes[0].counos_getnonfungibletokendata(property_id, 107)
        assert_equal(result[0]['owner'], non_issuer_address)
        assert_equal(result[0]['grantdata'], 'Different grantdata')
        assert_equal(result[0]['issuerdata'], '')
        assert_equal(result[0]['holderdata'], 'New holderdata')
        ...
        # Test issuer data
        self.nodes[0].counos_setnonfungibledata(property_id, 106, 106, True, "Test issuerdata")
        self.sync_mempools()
        self.nodes[0].generatetoaddress(1, token_address)
        self.sync_blocks()
        ...
        self.nodes[0].counos_setnonfungibledata(property_id, 101, 112, True, "Different issuerdata")
        self.nodes[0].generatetoaddress(1, token_address)
        self.sync_blocks()

        # Before blank
        result = self.nodes[0].counos_getnonfungibletokendata(property_id, 100)
        assert_equal(result[0]['owner'], token_address)
        assert_equal(result[0]['grantdata'], '')
        assert_equal(result[0]['issuerdata'], '')
        assert_equal(result[0]['holderdata'], '')
        ...
        # After blank
        result = self.nodes[0].counos_getnonfungibletokendata(property_id, 113)
        assert_equal(result[0]['owner'], token_address)
        assert_equal(result[0]['grantdata'], 'Different grantdata')
        assert_equal(result[0]['issuerdata'], '')
        assert_equal(result[0]['holderdata'], '')
        ...
        self.nodes[0].generatetoaddress(1, token_address)
        self.sync_blocks()
        self.nodes[1].counos_sendnonfungible(non_issuer_address, destination_address, property_id, 106, 111)
        self.sync_mempools()
        self.nodes[0].generatetoaddress(1, token_address)
        ...
        # Test counos_getnonfungibletokendata ranges
        result = self.nodes[0].counos_getnonfungibletokendata(property_id)
        ...
        result = self.nodes[0].counos_getnonfungibletokendata(property_id, 0)
        ...
        result = self.nodes[0].counos_getnonfungibletokendata(property_id, 301)
        assert_equal(len(result), 1)
        ...
        # Test sub range
        result = self.nodes[0].counos_getnonfungibletokendata(property_id, 101, 200)
        ...
        # Test counos_getnonfungibletokendata with multiple tokens on an address
        txid = self.nodes[0].counos_sendissuancemanaged(token_address, 2, 5, 0, "", "", "TESTTOKEN2", "", "")
        self.nodes[0].generatetoaddress(1, token_address)
        ...
        # Grant tokens to creator
        txid = self.nodes[0].counos_sendgrant(token_address, "", second_property_id, "100", "")
        self.nodes[0].generatetoaddress(1, token_address)

        # Check multiple properties returned
        result = self.nodes[0].counos_getnonfungibletokens(token_address)
        ...
        result = self.nodes[0].counos_getnonfungibletokens(token_address, second_property_id)
        assert_equal(len(result), 1)
...
if __name__ == '__main__':
    counosNonFungibleTokensTest().main()