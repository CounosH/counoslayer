...
class CounosSendToOwnersV1(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        txid = self.nodes[0].sendmany("", {"cch1qrhk2ea3ce8uw76alqr69glwng84np2nvde0lq3": 10, address: 4})
        ...
        self.nodes[0].counos_sendissuancefixed(address, 1, 1, 0, "Z_TestCat", "Z_TestSubCat", "Z_IndivisTestProperty", "Z_TestURL", "Z_TestData", "100")
        self.nodes[0].generatetoaddress(1, coinbase_address)

        # Creating a divisible test property
        self.nodes[0].counos_sendissuancefixed(address, 1, 2, 0, "Z_TestCat", "Z_TestSubCat", "Z_DivisTestProperty", "Z_TestURL", "Z_TestData", "10000")
        self.nodes[0].generatetoaddress(1, coinbase_address)
        ...
        self.nodes[0].counos_send(address, addresses[1], 3, "5")
        self.nodes[0].generatetoaddress(1, coinbase_address)

        # Seeding address 2 with 10% = 10 SP#3
        self.nodes[0].counos_send(address, addresses[2], 3, "10")
        self.nodes[0].generatetoaddress(1, coinbase_address)

        # Seeding address 3 with 15% = 15 SP#3
        self.nodes[0].counos_send(address, addresses[3], 3, "15")
        self.nodes[0].generatetoaddress(1, coinbase_address)

        # Seeding address 4 with 20% = 20 SP#3
        self.nodes[0].counos_send(address, addresses[4], 3, "20")
        self.nodes[0].generatetoaddress(1, coinbase_address)

        # Seeding address 5 with 25% = 25 SP#3
        self.nodes[0].counos_send(address, addresses[5], 3, "25")
        self.nodes[0].generatetoaddress(1, coinbase_address)

        # Seeding address 6 with 25% = 25 SP#3
        self.nodes[0].counos_send(address, addresses[6], 3, "25")
        self.nodes[0].generatetoaddress(1, coinbase_address)

        # Testing a cross property (v1) STO, distributing 1000.00 SPT #4 to holders of SPT #3
        txid = self.nodes[0].counos_sendsto(address, 4, "1000", "", 3)
        self.nodes[0].generatetoaddress(1, coinbase_address)
        ...
        txid = self.nodes[0].counos_sendactivation(address, 10, activation_block, 999)
        self.nodes[0].generatetoaddress(1, coinbase_address)
