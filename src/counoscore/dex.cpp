 ...
 */
static int64_t calculateDesiredCCH(const int64_t amountOffered, const int64_t amountDesired, const int64_t amountAvailable)
{
    ...
    CCH = amount_des * balanceReallyAvailable;
    CCH /= (double) nValue;
    amount_des = rounduint64(CCH);
...
/**
 * Determines the amount of counoshs desired, in case it needs to be recalculated.
 *
 * TODO: don't expose it!
 * @return The amount of counoshs desired
 */
int64_t calculateDesiredCCH(const int64_t amountOffered, const int64_t amountDesired, const int64_t amountAvailable)
{
    if (amountOffered == 0) {
...
int DEx_offerCreate(const std::string& addressSeller, uint32_t propertyId, int64_t amountOffered, int block, int64_t amountDesired, int64_t minAcceptFee, uint8_t paymentWindow, const uint256& txid, uint64_t* nAmended)
{
    ...
    if (amountOffered > balanceReallyAvailable) {
        ...
        // AND we must also re-adjust the CCH desired in this case...
        amountDesired = legacy::calculateDesiredCCH(amountOffered, amountDesired, balanceReallyAvailable);
        amountOffered = balanceReallyAvailable;
        if (nAmended) *nAmended = amountOffered;

        PrintToLog("%s: adjusting order: updated amount for sale: %s %s, offered for: %s CCH\n", __func__,
...
int DEx_acceptCreate(const std::string& addressBuyer, const std::string& addressSeller, uint32_t propertyId, int64_t amountAccepted, int block, int64_t feePaid, uint64_t* nAmended)
{
    ...
    // ensure the correct CCH fee was paid in this acceptance message
    ...
    if (amountReserved > 0) {
        ...
        CMPAccept acceptOffer(amountReserved, block, offer.getBlockTimeLimit(), offer.getProperty(), offer.getOfferAmountOriginal(), offer.getCCHDesiredOriginal(), offer.getHash());
...
static int64_t calculateDExPurchase(const int64_t amountOffered, const int64_t amountDesired, const int64_t amountPaid)
{
    ...
    uint64_t acceptCCHDesired = static_cast<uint64_t>(amountDesired);
    uint64_t CCH_paid = static_cast<uint64_t>(amountPaid);

    const double CCH_desired_original = acceptCCHDesired;
    const double offer_amount_original = acceptOfferAmount;

    double perc_X = (double) CCH_paid / CCH_desired_original;
...
/**
 * Handles incoming CCH payment for the offer.
 * TODO: change nAmended: uint64_t -> int64_t
 */
int DEx_payment(const uint256& txid, unsigned int vout, const std::string& addressSeller, const std::string& addressBuyer, int64_t amountPaid, int block, uint64_t* nAmended)
{
    ...
    const int64_t amountDesired = p_accept->getCCHDesiredOriginal();
    ...
    if (msc_debug_dex) PrintToLog(
            "%s: CCH desired: %s, offered amount: %s, amount to purchase: %s, amount remaining: %s\n", __func__,
