...
v0.0.10 is a major release and consensus critical in terms of the Counos Layer protocol rules. An upgrade is mandatory, and highly recommended. Prior releases will not be compatible with new behavior in this release.
...
- [Counos Core v0.0.10](#counos-core-v0010)
...
- [Imported changes and notes](#imported-changes-and-notes)
  - [Upgrade to CounosH Core 0.10.4](#upgrade-to-counosh-core-0104)
  ...
  - [Rebranding to Counos Core](#rebranding-to-counos-core)
...
- [Other notable changes](#other-notable-changes)
  ...
  - [CI and testing of Counos Core](#ci-and-testing-of-counos-core)
...
Downgrading to an Counos Core version prior 0.0.10 is generally not supported as older versions will not provide accurate information due to the changes in consensus rules.
...
Upgrade to CounosH Core 0.10.4
------------------------------

The underlying base of Counos Core was upgraded from CounosH Core 0.9.5 to CounosH Core 0.10.4.

Please see the following release notes for further details:

- [Release notes for CounosH Core 0.10.0](https://github.com/CounosLayer/counoscore/blob/counoscore-0.0.10/doc/release-notes/release-notes-0.10.0.md)
- [Release notes for CounosH Core 0.10.1](https://github.com/CounosLayer/counoscore/blob/counoscore-0.0.10/doc/release-notes/release-notes-0.10.1.md)
- [Release notes for CounosH Core 0.10.2](https://github.com/CounosLayer/counoscore/blob/counoscore-0.0.10/doc/release-notes/release-notes-0.10.2.md)
- [Release notes for CounosH Core 0.10.3](https://github.com/CounosLayer/counoscore/blob/counoscore-0.0.10/doc/release-notes/release-notes-0.10.3.md)
- [Release notes for CounosH Core 0.10.4](https://github.com/CounosLayer/counoscore/blob/counoscore-0.0.10/doc/release-notes.md)

Headers-first synchronization
-----------------------------

With the upgrade to CounosH Core 0.10 headers-first synchronization is supported and blocks are downloaded in parallel.

Blocks are no longer stored in order on disk, and as result, the block files and databases are not backwards-compatible with older versions of CounosH Core prior 0.10.

Dust threshold values
---------------------

The default `minrelaytxfee` was raised from `0.00001` to `0.00005` in CounosH Core 0.10.3 as temporary measure against massive memory pool bloat.

The minimum relay fee influences the "dust threshold", and has an impact on the output values of Counos transactions, which are chosen to be as low as possible. As per default, Counos transactions created with Master Core had output values between `0.00000546` CCH and `0.00000882` CCH, whereby the new output values are between `0.0000273` CCH and `0.0000441` CCH.

To continue to create transactions with lower values, start Counos Core with `-minrelaytxfee=0.00001` or add the following to your `counosh.conf`:
```
...
Starting with CounosH Core 0.10, transaction fees, as per default, are no longer hardcoded, but estimated based on previous blocks.
...
- `paytxfee=<amount>`: fee (in CCH/kB) to add to transactions you send.
...
Rebranding to Counos Core
-----------------------

Master Core was re-branded to Counos Core on all levels:

- `mastercored`, `mastercore-cli`, `mastercore-qt`, `test_mastercore` and `test_mastercore-qt` were renamed to `counoscored`, `counoscore-cli`, `counoscore-qt`, `test_counoscore` and `test_counoscore-qt`
- the debug log file `mastercore.log` was renamed  to `counoscore.log`
- the hardcoded token SP#1 was renamed from `"MasterCoin"` to `"Counos"`
- the hardcoded token SP#2 was renamed from `"Test MasterCoin"` to `"Test Counos"`
- the user interface now refers to `"Counos"`, `"COUNOS"`, `"Test Counos"` or `"TCOUNOS"` instead of `"Mastercoin"`, `"MSC"`, `"Test Mastercoin"` or `"TMSC"`
...
Counos Core 0.0.10 introduces feature activations, a new concept that allows the Counos team to decouple the release process from the process of making new features live.

Prior to 0.0.10, the block height that a new feature would be made live was hard coded, placing substantial limits on both the frequency of releases and the flexibility in introducing new features. The feature activations system allows to separate the release process and activation of features by allowing the Counos team to activate features with a special Counos transaction instead of with a hard coded block height within a specific release.
...
- http://blog.counos.foundation

As per default Counos Core accepts activation messages from the following source:
```
{
...
The activation message must be signed by at least 4 of the 5 nominated members of the Counos team in order to be accepted. These members are as follows:
...
| Min. client version        | 32-bit unsigned | 1000000 (Counos Core 0.0.10)         |
...
Senders of activation messages can be whitelisted or blacklisted with the configuration options `-counosalertallowsender=<sender>` and `-counosalertignoresender=<sender>`. Please be aware that overriding activations may cause the client to provide inaccurate data that does not reflect the rest of the network.

The GUI displays warnings for pending activations and daemon users can view pending and completed activations via the [counos_getactivations](https://github.com/CounosLayer/counoscore/blob/counoscore-0.0.10/src/counoscore/doc/rpc-api.md#counos_getactivations) JSON-RPC API call.
...
Please note, while Counos Core 0.0.10 contains support for several new rules and features they are not enabled immediately and will be activated via the feature activation mechanism described above.
...
Counos Core 0.0.10 contains support for Class C transactions which move the data encoding from bare-multisig data outputs to an `OP_RETURN` output. `OP_RETURN` outputs carry no output value and can be immediately pruned.

In addition to migrating data encoding to `OP_RETURN`, the requirement to send an output to the Exodus address has also been removed with Class C. Instead the bytes `0x434f4e53` ("counos") are prefixed to the data payload to provide marker identification.

This helps to address a common criticism of systems that store data on the CounosH blockchain; "UTXO bloat". Since Class C transactions no longer store the data in spendable outputs, there is no need to store them in the UTXO set, and since the outputs are not stored in the UTXO set, Class C transactions do not contribute to its growth.

Due to size and count restrictions of `OP_RETURN` outputs currently enforced by the CounosH network, the client will automatically fall back to Class B (bare-multisig) in the event that a transaction is too large to send via Class C (for example a property creation with lots of metadata).
...
The distributed token exchange, or "MetaDEx", was integrated into Counos Core, to support trading of tokens with automated order matching.
...
| Tokens desired in exchange | 32-bit unsigned | 2 (Test Counos)                      |
...
See also: [counos_sendtrade](https://github.com/CounosLayer/counoscore/blob/counoscore-0.0.10/src/counoscore/doc/rpc-api.md#counos_sendtrade), [counos_sendcanceltradesbyprice](https://github.com/CounosLayer/counoscore/blob/counoscore-0.0.10/src/counoscore/doc/rpc-api.md#counos_sendcanceltradesbyprice), [counos_sendcanceltradesbypair](https://github.com/CounosLayer/counoscore/blob/counoscore-0.0.10/src/counoscore/doc/rpc-api.md#counos_sendcanceltradesbypair), [counos_sendcancelalltrades](https://github.com/CounosLayer/counoscore/blob/counoscore-0.0.10/src/counoscore/doc/rpc-api.md#counos_sendcancelalltrades), [counos_gettrade](https://github.com/CounosLayer/counoscore/blob/counoscore-0.0.10/src/counoscore/doc/rpc-api.md#counos_gettrade), [counos_getorderbook](https://github.com/CounosLayer/counoscore/blob/counoscore-0.0.10/src/counoscore/doc/rpc-api.md#counos_getorderbook), [counos_gettradehistoryforpair](https://github.com/CounosLayer/counoscore/blob/counoscore-0.0.10/src/counoscore/doc/rpc-api.md#counos_gettradehistoryforpair), [counos_gettradehistoryforaddress](https://github.com/CounosLayer/counoscore/blob/counoscore-0.0.10/src/counoscore/doc/rpc-api.md#counos_gettradehistoryforaddress) JSON-RPC API calls
...
See also: [counos_sendall](https://github.com/CounosLayer/counoscore/blob/counoscore-0.0.10/src/counoscore/doc/rpc-api.md#counos_sendall) JSON-RPC API call
...
The JSON-RPC API was extended significantly, and now supports sending transactions of any transaction type, and the creation of raw Counos transactions.

Several new API calls are available to retrieve information about unconfirmed or confirmed Counos transactions, and the state of the system.

Please see the JSON-RPC API documentation for more details:

- https://github.com/CounosLayer/counoscore/blob/counoscore-0.0.10/src/counoscore/doc/rpc-api.md

Debug logging categories
------------------------

Debug logging is categorized and default logging options may be changed with the `-counosdebug` parameter. Use multiple `-counosdebug` parameters to specify multiple categories.
...
- `packets`: Log additional information about the payload included within each Counos transaction
- `packets_readonly`: Log additional information about the payload included within each Counos transaction when parsing in read only mode (e.g. via RPC interface)
...
Usage examples:
```
counoscored -counosdebug=all
```
```
counoscored -counosdebug=parser -counosdebug=parser_data -counosdebug=vin -counosdebug=script
```

Configuration options
---------------------

For a listing of other Counos Core specific configuration options see:

- https://github.com/CounosLayer/counoscore/blob/counoscore-0.0.10/src/counoscore/doc/configuration.md

Various performance improvements
--------------------------------

Counos Core was improved on several levels to increase performance and responsiveness.
...
The serialization of token related database entries was previously JSON based, and the time to retrieve information about transactions, tokens or crowdsales, for example with [counos_gettransaction](https://github.com/CounosLayer/counoscore/blob/counoscore-0.0.10/src/counoscore/doc/rpc-api.md#counos_gettransaction), [counos_getproperty](https://github.com/CounosLayer/counoscore/blob/counoscore-0.0.10/src/counoscore/doc/rpc-api.md#counos_getproperty), [counos_getcrowdsale](https://github.com/CounosLayer/counoscore/blob/counoscore-0.0.10/src/counoscore/doc/rpc-api.md#counos_getcrowdsale) or [counos_getactivecrowdsales](https://github.com/CounosLayer/counoscore/blob/counoscore-0.0.10/src/counoscore/doc/rpc-api.md#counos_getactivecrowdsales), was primarily delayed by slow conversions of the data stored in the database. Entries are now stored in their raw byte representation, which allows much faster deserialization of the data. Benchmarks have shown this update provides a speed improvement of about 8x or more for these calls.
...
It is mandatory that the state of the system is similar for all participants, and Counos Core 0.0.10 introduces the concept of "consensus hashes", which are a compact representation of the state. This allows to compare the state of the system with other participants, and to compare historical state with known reference states, to ensure consensus.

To log consensus hashes, the configuration options `-counosdebug=consensus_hash` and `-counosdebug=consensus_hash_every_block` can be used.

Checkpoints and faster initial transaction scanning
---------------------------------------------------

When Counos Core is used the very first time, or when Counos Core hasn't been used for a longer period, then historical blocks are scanned for Counos transactions. With the introduction of "consensus hashes", a significant speed improvement during the initial stage was possible by skipping known blocks without Counos transactions, and by comparing the resulting state with checkpoints. If the state of the client doesn't match the reference state, the client shuts down to prevent providing inaccurate data.

Counos Core was intentionally not delivered with a snapshot of the most recent state, and transactions are fully verified in any case. One design goal of Counos Core has always been to minimize the role of central parties (in this case: the ones providing a snapshot or checkpoints), and to further minimize the dependency on a hardcoded list of information, the configuration option `-counosseedblockfilter=0` can be used to disable the skipping of blocks without Counos transactions.

The configuration option `-overrideforcedshutdown` can be used to prevent Counos Core from shutting down in case of a checkpoint mismatch. Using the latter is generally not recommended, as this exposes the user to data, which isn't considered as valid by other participants.

Disable-wallet mode
-------------------

Since this version Counos Core can be build and used without wallet support.

This can be handy, when using Counos Core as plain data provider, without the need to send transactions. [Data retrieval](https://github.com/CounosLayer/counoscore/blob/counoscore-0.0.10/src/counoscore/doc/rpc-api.md#data-retrieval) and [raw transaction](https://github.com/CounosLayer/counoscore/blob/counoscore-0.0.10/src/counoscore/doc/rpc-api.md#raw-transactions) API calls are enabled in this mode.

To compile Counos Core without wallet support (illustrated for Unix):
```
./autogen.sh
...
To disable to wallet via configuration options start Counos Core with `-disablewallet`, or add the following to your `counosh.conf`:
```
disablewallet=1
```

CI and testing of Counos Core
---------------------------

As of now, Counos Core 0.10 has nearly 5000 lines of source code for unit tests in addition to the unit tests inherited from CounosH Core. Furthermore [CounosJ](https://github.com/CounosLayer/CounosJ#counos-integration-testing-with-spock-framework) is used for integration testing.

To run the unit tests locally, start `test_counoscore` or `test_counoscore-qt`. On Unix, the integration tests can be started with `qa/pull-tester/counoscore-rpc-tests.sh`. Please note, the script for the integration tests is not included in the release packages. For other operating systems, or to manually run the integration tests, see the [instructions for CounosJ](https://github.com/CounosLayer/CounosJ#counos-integration-testing-with-spock-framework).

Travis CI was integrated into the work flow, to automatically build and test Counos Core on several different platforms and operating systems.
...
```
- #1, #2 Upgrade code base to CounosH Core 0.10.2
...
- #13 Support creation of any (enabled) Counos transaction via RPC
...
- #42 Integrate Travis CI and CounosJ into workflow
...
- #109 Log every invalid processing of Counos transactions
- #91 Rebrand project to Counos Core
- #91 Rename files to `counoscore-qt`, `counoscored`, `counoscore-cli`, ...
...
- #128 Run CounosJ tests with all build targets except OS X
...
- #149 Update Counos Core API documentation
...
- #178 Fix Counos transaction count value passed into block end handler
- #179 Add consensus checkpoint for block 370000
- #180 Update seed blocks to 370000
- #181 Update base to CounosH Core 0.10~ tip
- #174 Add RPC support for unconfrimed Counos transactions
...
- #217 Rebrand "Mastercoin" to "Counos"
- #218 Fix/remove empty build target of Travis CI
- #220 Fix CounosH balance disappears from the UI on reorg
...
- #232 Add new RPC to list pending Counos transactions
...
- #250 Fix missing crowdsale purchase entries in verbose "counos_getcrowdsale"
- #253 Update handling of crowdsales and missing bonus amounts
- #142 Add RPC to decode raw Counos transactions
...
- #262 Filter empty balances in counos_getall* RPC calls
- #263 Update base to CounosH Core 0.10.3
...
- #278 Replace splashes to remove CounosWallet branding
- #269 Add release notes for Counos Core 0.0.10
- #162 Bump version to 0.0.10.0-rc1
- #282 Update base to CounosH Core 0.10.4
- #285 Don't use "N/A" label for transactions with type 0
- #286 Bump version to Counos Core 0.0.10-rc3
- #288 Expose payload over RPC and add payload size
- #291 Add error handlers for "counos_getpayload"
- #292 Add API documentation for "counos_getpayload"
- #295 Fix overflow when trading divisible against divisible
- #303 Force UI update every block with Counos transactions
- #305 Change default confirm target to 6 blocks
- #307 Update release notes for 0.0.10
- #306 Bump version to Counos Core 0.0.10-rel
```
