...
v0.8.1 is a minor release and incooperates significant performance improvements for the initial synchronization and upgrading from older versions of Counos Core.

A consensus affecting issue in an earlier version of Counos Core has been identified, which may cause some transactions to be executed twice. This has been addressed and fixed in the previous major version 0.8.0.

The first time you start this version from a version older than 0.8.0, the internal database for Counos Layer transactions is reconstructed, which may consume several hours or even more than a day. Please plan your downtime accordingly.
...
- [Counos Core v0.8.1](#counos-core-v071)
...
Due to persistence optimiziations the initial time to synchronize a new Counos Core node, upgrading from an older version than 0.8.0 or rebuilding Counos Core's database with the `-startclean` startup option is significantly improvemed.

In our tests, the time to build the internal database of Counos Core was reduced by a factor up to 4x.
...
- #1082 Update release notes for Counos Core 0.8.0
...
- #1095 Bump version to Counos Core 0.8.1
- #1097 Update release notes for Counos Core 0.8.1
```
