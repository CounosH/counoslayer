...
std::string mastercore::strTransactionType(uint16_t txType)
{
    switch (txType) {
        ...
        case MSC_TYPE_ACCEPT_OFFER_CCH: return "DEx Accept Offer";
        ...
        case COUNOSCORE_MESSAGE_TYPE_ALERT: return "ALERT";
        case COUNOSCORE_MESSAGE_TYPE_DEACTIVATION: return "Feature Deactivation";
        case COUNOSCORE_MESSAGE_TYPE_ACTIVATION: return "Feature Activation";
...
bool CMPTransaction::interpret_Transaction()
{
    ...
    switch (type) {
        ...
        case MSC_TYPE_ACCEPT_OFFER_CCH:
            return interpret_AcceptOfferCCH();
        ...
        case COUNOSCORE_MESSAGE_TYPE_DEACTIVATION:
        ...
        case COUNOSCORE_MESSAGE_TYPE_ALERT:
...
/** Tx 22 */
bool CMPTransaction::interpret_AcceptOfferCCH()
{
    if (pkt_size < 16) {
        return false;
    }
...
int CMPTransaction::interpretPacket()
{
    ...
    switch (type) {
        ...
        case MSC_TYPE_ACCEPT_OFFER_CCH:
            return logicMath_AcceptOffer_CCH();
...
int CMPTransaction::logicMath_SendToOwners()
{
    ...
    uint32_t feeProperty = isTestEcosystemProperty(property) ? COUNOS_PROPERTY_TMSC : COUNOS_PROPERTY_MSC;
    ...
    // Number of tokens has changed, update fee distribution thresholds
    if (version == MP_TX_PKT_V0) NotifyTotalTokensChanged(COUNOS_PROPERTY_MSC, block); // fee was burned
...
int CMPTransaction::logicMath_SendAll()
{
    ...
    while (0 != (propertyId = ptally->next())) {
        ...
        if (ecosystem == COUNOS_PROPERTY_MSC && isTestEcosystemProperty(propertyId)) {
            continue;
        }
        if (ecosystem == COUNOS_PROPERTY_TMSC && isMainEcosystemProperty(propertyId)) {
            continue;
        }
...
int CMPTransaction::logicMath_TradeOffer()
{
    ...
    // Ensure only COUNOS and TCOUNOS are allowed, when the DEx is not yet free
    if (!IsFeatureActivated(FEATURE_FREEDEX, block)) {
        if (COUNOS_PROPERTY_TMSC != property && COUNOS_PROPERTY_MSC != property) {
            PrintToLog("%s(): rejected: property for sale %d must be COUN or TCOUN\n", __func__, property);
...
/** Tx 22 */
int CMPTransaction::logicMath_AcceptOffer_CCH()
{
    if (!IsTransactionTypeAllowed(block, property, type, version)) {
        PrintToLog("%s(): rejected: type %d or version %d not permitted for property %d at block %d\n",
                __func__,
...
int CMPTransaction::logicMath_MetaDExTrade()
{
    ...
    if (!IsFeatureActivated(FEATURE_TRADEALLPAIRS, block)) {
        // Trading non-Counos pairs is not allowed before trading all pairs is activated
        if ((property != COUNOS_PROPERTY_MSC) && (desired_property != COUNOS_PROPERTY_MSC) &&
            (property != COUNOS_PROPERTY_TMSC) && (desired_property != COUNOS_PROPERTY_TMSC)) {
            PrintToLog("%s(): rejected: one side of a trade [%d, %d] must be COUN or TCOUN\n", __func__, property, desired_property);
...
int CMPTransaction::logicMath_MetaDExCancelEcosystem()
{
    ...
    if (COUNOS_PROPERTY_MSC != ecosystem && COUNOS_PROPERTY_TMSC != ecosystem) {
...
int CMPTransaction::logicMath_EnableFreezing(CBlockIndex* pindex)
{
    ...
    if (!IsFeatureActivated(FEATURE_FREEZENOTICE, block)) {
        ...
        liveBlock = params.COUNOS_FREEZE_WAIT_PERIOD + block;
    }
...
int CMPTransaction::logicMath_Alert()
{
    ...
    if (alert_type == ALERT_CLIENT_VERSION_EXPIRY && COUNOSCORE_VERSION < alert_expiry) {
