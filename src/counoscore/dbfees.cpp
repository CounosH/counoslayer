 ...
 * @file dbfees.cpp
 *
 * This file contains code for handling Counos fees.
 */
...
CCounosFeeCache::CCounosFeeCache(const fs::path& path, bool fWipe)
{
    leveldb::Status status = Open(path, fWipe);
    PrintToConsole("Loading fee cache database: %s\n", status.ToString());
}

CCounosFeeCache::~CCounosFeeCache()
{
    if (msc_debug_fees) PrintToLog("CCounosFeeCache closed\n");
}

// Returns the distribution threshold for a property
int64_t CCounosFeeCache::GetDistributionThreshold(const uint32_t &propertyId)
{
...
void CCounosFeeCache::UpdateDistributionThresholds(uint32_t propertyId)
{
    int64_t distributionThreshold = getTotalTokens(propertyId) / COUNOS_FEE_THRESHOLD;
...
int64_t CCounosFeeCache::GetCachedAmount(const uint32_t &propertyId)
{
    assert(pdb);
...
void CCounosFeeCache::ClearCache(const uint32_t &propertyId, int block)
{
...
void CCounosFeeCache::AddFee(const uint32_t &propertyId, int block, const int64_t &amount)
{
...
void CCounosFeeCache::RollBackCache(int block)
{
    assert(pdb);
...
void CCounosFeeCache::EvalCache(const uint32_t &propertyId, int block)
{
...
void CCounosFeeCache::DistributeCache(const uint32_t &propertyId, int block)
{
    LOCK(cs_tally);
    ...
    if (isTestEcosystemProperty(propertyId)) {
        receiversSet = STO_GetReceivers("FEEDISTRIBUTION", COUNOS_PROPERTY_TMSC, cachedAmount);
    } else {
        receiversSet = STO_GetReceivers("FEEDISTRIBUTION", COUNOS_PROPERTY_MSC, cachedAmount);
    }

    uint64_t numberOfReceivers = receiversSet.size(); // there will always be addresses holding COUNOS, so no need to check size>0
...
void CCounosFeeCache::PruneCache(const uint32_t &propertyId, int block)
{
...
void CCounosFeeCache::printStats()
{
    PrintToConsole("CCounosFeeCache stats: nWritten= %d , nRead= %d\n", nWritten, nRead);
}

// Show Fee Cache DB records
void CCounosFeeCache::printAll()
{
    int count = 0;
...
std::set<feeCacheItem> CCounosFeeCache::GetCacheHistory(const uint32_t &propertyId)
{
    assert(pdb);
...
CCounosFeeHistory::CCounosFeeHistory(const fs::path& path, bool fWipe)
{
    leveldb::Status status = Open(path, fWipe);
    PrintToConsole("Loading fee history database: %s\n", status.ToString());
}

CCounosFeeHistory::~CCounosFeeHistory()
{
    if (msc_debug_fees) PrintToLog("CCounosFeeHistory closed\n");
}

// Show Fee History DB statistics
void CCounosFeeHistory::printStats()
{
    PrintToConsole("CCounosFeeHistory stats: nWritten= %d , nRead= %d\n", nWritten, nRead);
}

// Show Fee History DB records
void CCounosFeeHistory::printAll()
{
    int count = 0;
    leveldb::Iterator* it = NewIterator();
    for(it->SeekToFirst(); it->Valid(); it->Next()) {
...
int CCounosFeeHistory::CountRecords()
{
...
void CCounosFeeHistory::RollBackHistory(int block)
{
    assert(pdb);
...
std::set<int> CCounosFeeHistory::GetDistributionsForProperty(const uint32_t &propertyId)
{
    assert(pdb);
...
bool CCounosFeeHistory::GetDistributionData(int id, uint32_t *propertyId, int *block, int64_t *total)
{
    assert(pdb);
...
std::set<feeHistoryItem> CCounosFeeHistory::GetFeeDistribution(int id)
{
    assert(pdb);
...
void CCounosFeeHistory::RecordFeeDistribution(const uint32_t &propertyId, int block, int64_t total, std::set<feeHistoryItem> feeRecipients)
{
    assert(pdb);
