...
std::vector<TransactionRestriction> CConsensusParams::GetRestrictions() const
{
    ...
    { //  transaction type                    version        allow 0  activation block
      //  ----------------------------------  -------------  -------  ------------------
        { COUNOSCORE_MESSAGE_TYPE_ALERT,        0xFFFF,        true,    MSC_ALERT_BLOCK    },
        { COUNOSCORE_MESSAGE_TYPE_ACTIVATION,   0xFFFF,        true,    MSC_ALERT_BLOCK    },
        { COUNOSCORE_MESSAGE_TYPE_DEACTIVATION, 0xFFFF,        true,    MSC_ALERT_BLOCK    },
        ...
        { MSC_TYPE_ACCEPT_OFFER_CCH,          MP_TX_PKT_V0,  false,   MSC_DEX_BLOCK      },
...
std::vector<ConsensusCheckpoint> CMainConsensusParams::GetCheckpoints() const
{
    ...
    const ConsensusCheckpoint vCheckpoints[] = {
          { 100000, uint256S("00000000000fe315bcdfdbf02316f1231364fda6bb1a494a779220a19ca2fa98"),
                    uint256S("c9044511a36fc4f2ea9a31bd7728da2a046c4e95dec60b6aec8718cb87cf8737") },
    };
...
std::vector<TransactionCheckpoint> CMainConsensusParams::GetTransactions() const
{
    ...
    { 106679, uint256S("f8d36985f6c3ae9e89179e6fda505f42675f4ed8a286096706def654167d60e8") },
    { 106681, uint256S("0ad88d7c7b8a5a2f2a8007d0505793abfa87fb7803dbfd3632ec62857bb00cf1") },
    };
...
/**
 * Constructor for mainnet consensus parameters.CMainConsensusParams::CMainConsensusParams()
 */
CTestNetConsensusParams::CTestNetConsensusParams()
{
    ...
    COUNOS_FREEZE_WAIT_PERIOD = 4096; // ~4 weeks
    // Script related:
    PUBKEYHASH_BLOCK = 0;
    SCRIPTHASH_BLOCK = 60000;
    MULTISIG_BLOCK = 0;
...
/**
 * Constructor for testnet consensus parameters.CTestNetConsensusParams::CTestNetConsensusParams()
 */
...
/**
 * Constructor for regtest consensus parameters. CRegTestConsensusParams
 */
...
//PrintToLog("whichType %d:\n",whichType);
//PrintToLog("nBlock %d:\n",nBlock);

    switch (whichType)
    {
        case TX_PUBKEYHASH:
//            PrintToLog("PUBKEYHASH_BLOCK %d:\n",params.PUBKEYHASH_BLOCK);
//            PrintToLog("TX_PUBKEYHASH %d:\n",TX_PUBKEYHASH);
            return (params.PUBKEYHASH_BLOCK <= nBlock);

        case TX_SCRIPTHASH:
//        PrintToLog("SCRIPTHASH_BLOCK %d:\n",params.SCRIPTHASH_BLOCK);
//        PrintToLog("TX_SCRIPTHASH %d:\n",TX_SCRIPTHASH);
            return (params.SCRIPTHASH_BLOCK <= nBlock);

//start add to send for counosh
        case TX_WITNESS_V0_KEYHASH:
//        PrintToLog("TX_WITNESS_V0_KEYHASH %d:\n",TX_WITNESS_V0_KEYHASH);
            return (params.PUBKEYHASH_BLOCK <= nBlock);

        case TX_WITNESS_V0_SCRIPTHASH:
//        PrintToLog("TX_WITNESS_V0_SCRIPTHASH %d:\n",TX_WITNESS_V0_SCRIPTHASH);
            return (params.SCRIPTHASH_BLOCK <= nBlock);
//end
    }
...
bool ActivateFeature(uint16_t featureId, int activationBlock, uint32_t minClientVersion, int transactionBlock)
{
    ...
    bool supported = COUNOSCORE_VERSION >= minClientVersion;
    switch (featureId) {
    ...
    if (!supported) {
        ...
        AddAlert("counoscore", ALERT_BLOCK_EXPIRY, activationBlock, alertText);
        DoWarning(alertText);
    }
...
bool DeactivateFeature(uint16_t featureId, int transactionBlock)
{
    ...
    AddAlert("counoscore", ALERT_BLOCK_EXPIRY, transactionBlock + 1024, alertText);
    DoWarning(alertText);
...
std::string GetFeatureName(uint16_t featureId)
{
    switch (featureId) {
        ...
        case FEATURE_FEES: return "Fee system (inc 0.05% fee from trades of non-Counos pairs)";
 ...
 *
 * Certain transactions use a property identifier of 0 (= CCH) as wildcard, which
...
bool IsTransactionTypeAllowed(int txBlock, uint32_t txProperty, uint16_t txType, uint16_t version)
{
    ...
    for (std::vector<TransactionRestriction>::const_iterator it = vTxRestrictions.begin(); it != vTxRestrictions.end(); ++it)
    {
        ...
        // a property identifier of 0 (= CCH) may be used as wildcard
        if (COUNOS_PROPERTY_CCH == txProperty && !entry.allowWildcard) {
            continue;
        }
...
{
//    PrintToLog("blockasdddddddjhdsiopuhgduiohgidshiouhuoihoiuf %d:\n",block);
