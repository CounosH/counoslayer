...
CCounosTransactionDB::CCounosTransactionDB(const fs::path& path, bool fWipe)
{
    leveldb::Status status = Open(path, fWipe);
    PrintToConsole("Loading master transactions database: %s\n", status.ToString());
}

CCounosTransactionDB::~CCounosTransactionDB()
{
    if (msc_debug_persistence) PrintToLog("CCounosTransactionDB closed\n");
}

/**
 * Retrieves the serialized transaction details from the DB. 
 */
std::vector<std::string> CCounosTransactionDB::FetchTransactionDetails(const uint256& txid)
{
    assert(pdb);
    std::string strValue;
    ...
    if (status.ok()) {
        ...
        if (vStr.size() == 2) {
            ...
            PrintToLog("ERROR: Entry (%s) found in CounosTXDB with unexpected number of attributes!\n", txid.GetHex());
        }
    } else {
        PrintToLog("ERROR: Entry (%s) could not be loaded from CounosTXDB!\n", txid.GetHex());
    }
 ...
 */
void CCounosTransactionDB::RecordTransaction(const uint256& txid, uint32_t posInBlock, int processingResult)
{
    assert(pdb);
 ...
 */
uint32_t CCounosTransactionDB::FetchTransactionPosition(const uint256& txid)
{
 ...
 */
std::string CCounosTransactionDB::FetchInvalidReason(const uint256& txid)
{
