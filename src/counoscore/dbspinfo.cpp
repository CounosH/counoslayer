...
CMPSPInfo::CMPSPInfo(const fs::path& path, bool fWipe)
{
    ...
    // special cases for constant SPs COUN and TCOUN
    implied_counos.issuer = EncodeDestination(ExodusAddress());
    implied_counos.updateIssuer(0, 0, implied_counos.issuer);
    implied_counos.prop_type = MSC_PROPERTY_TYPE_DIVISIBLE;
    implied_counos.num_tokens = 700000;
    implied_counos.category = "N/A";
    implied_counos.subcategory = "N/A";
    implied_counos.name = "Counos tokens";
    implied_counos.url = "https://counos.io/counoslayer";
    implied_counos.data = "Counos tokens serve as the binding between CounosH, smart properties and contracts created on the Counos Layer.";

    implied_tcounos.issuer = EncodeDestination(ExodusAddress());
    implied_tcounos.updateIssuer(0, 0, implied_tcounos.issuer);
    implied_tcounos.prop_type = MSC_PROPERTY_TYPE_DIVISIBLE;
    implied_tcounos.num_tokens = 700000;
    implied_tcounos.category = "N/A";
    implied_tcounos.subcategory = "N/A";
    implied_tcounos.name = "Test Counos tokens";
    implied_tcounos.url = "https://counos.io/counoslayer";
    implied_tcounos.data = "Test Counos tokens serve as the binding between CounosH, smart properties and contracts created on the Counos Layer.";
...
uint32_t CMPSPInfo::peekNextSPID(uint8_t ecosystem) const
{
    ...
    switch (ecosystem) {
        case COUNOS_PROPERTY_MSC: // Main ecosystem, MSC: 1, TMSC: 2, First available SP = 3
            nextId = next_spid;
            break;
        case COUNOS_PROPERTY_TMSC: // Test ecosystem, same as above with high bit set
...
bool CMPSPInfo::updateSP(uint32_t propertyId, const Entry& info)
{
    ...
    if (COUNOS_PROPERTY_MSC == propertyId || COUNOS_PROPERTY_TMSC == propertyId) {
        return false;
    }
...
uint32_t CMPSPInfo::putSP(uint8_t ecosystem, const Entry& info)
{
    ...
    // atomically write both the SP and the index to the database
    leveldb::WriteBatch batch;
    batch.Put(slSpKey, slSpValue);
...
bool CMPSPInfo::getSP(uint32_t propertyId, Entry& info) const
{
    ...
    if (COUNOS_PROPERTY_MSC == propertyId) {
        info = implied_counos;
        return true;
    } else if (COUNOS_PROPERTY_TMSC == propertyId) {
        info = implied_tcounos;
        return true;
    }
...
void CMPSPInfo::printAll() const
{
    ...
    for (uint32_t idx = COUNOS_PROPERTY_MSC; idx <= COUNOS_PROPERTY_TMSC; idx++) {
        Entry info;
