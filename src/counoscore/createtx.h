#ifndef COUNOSH_COUNOSCORE_CREATETX_H
#define COUNOSH_COUNOSCORE_CREATETX_H
...
// TODO:
// CMutableTransaction tx = CounosTxBuilder().addInput(input).addOpReturn(data).build();
// ... currently doesn't work, because addInput() returns a reference to a TxBuilder,
// but not to an CounosTxBuilder.
...
/**
 * Builder to create Counos transactions.
 *
 * The CounosTxBuilder class is an extension of the TxBuilder, with additional
 * methods to build Counos transactions. Payloads can be embedded with class B
 ...
 *   CMutableTransaction modifiedTx = CounosTxBuilder(basisTx)
 *           .addMultisig(payload, obfuscationSeed, redeemingPubKey)
 *           .build();
 * @endcode
 */
class CounosTxBuilder: public TxBuilder
{
public:
    /**
     * Creates a new Counos transaction builder.
     */
    CounosTxBuilder();

    /**
     * Creates a new Counos transaction builder to extend a transaction.
     *
     * @param transactionIn The transaction used to build upon
     */
    CounosTxBuilder(const CMutableTransaction& transactionIn);
     ...
     * @param outPoint The transaction outpoint to add
     */
    CounosTxBuilder& addInputs(const std::vector<PrevTxsEntry>& prevTxs);
     ...
     */
    CounosTxBuilder& addReference(const std::string& destination, int64_t value = 0);
     ...
     */
    CounosTxBuilder& addOpReturn(const std::vector<unsigned char>& data);
     ...
     */
    CounosTxBuilder& addMultisig(const std::vector<unsigned char>& data, const std::string& seed, const CPubKey& pubKey);
     ...
     */
    CounosTxBuilder& addChange(const std::string& destination, const CCoinsViewCache& view, int64_t txFee, uint32_t position = 0);
};
...
#endif // COUNOSH_COUNOSCORE_CREATETX_H
