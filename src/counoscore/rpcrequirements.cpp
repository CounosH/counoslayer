...
void RequirePrimaryToken(uint32_t propertyId)
{
    if (propertyId < 1 || 2 < propertyId) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Property identifier must be 1 (COUN) or 2 (TCOUN)");
    }
}
...
{
    if ((0.01 * COIN) < amount) {
        throw JSONRPCError(RPC_TYPE_ERROR, "Reference amount higher is than 0.01 CCH");
    }
}
...
void RequireSaneDExFee(const std::string& address, uint32_t propertyId)
{
    ...
    if (poffer->getMinFee() > 1000000) {
        throw JSONRPCError(RPC_TYPE_ERROR, "Minimum accept fee is higher than 0.01 CCH (use override = true to continue)");
    }
}
