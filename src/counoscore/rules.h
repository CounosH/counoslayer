#ifndef COUNOSH_COUNOSCORE_RULES_H
#define COUNOSH_COUNOSCORE_RULES_H
...
//! Feature identifier to enable non-Counos pairs on the distributed exchange
const uint16_t FEATURE_TRADEALLPAIRS = 8;
//! Feature identifier to enable the fee cache and strip 0.05% fees from non-Counos pairs
...
struct TransactionRestriction
{
    ...
    //! Transaction version
    uint16_t txVersion;
    //! Whether the property identifier can be 0 (= CCH)
    bool allowWildcard;
...
public:
    ...
    //! Block to enable trading of non-Counos pairs
    int TRADEALLPAIRS_FEATURE_BLOCK;
    //! Block to enable the fee system & 0.05% fee for trading non-Counos pairs
...
#endif // COUNOSH_COUNOSCORE_RULES_H
