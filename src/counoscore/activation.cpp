...
void CheckLiveActivations(int blockHeight)
{
    ...
    for (std::vector<FeatureActivation>::iterator it = vecPendingActivations.begin(); it != vecPendingActivations.end(); ++it) {
        ...
        if (COUNOSCORE_VERSION < liveActivation.minClientVersion) {
...
/**
 * Determines whether the sender is an authorized source for Counos Core feature activation.
 *
 * The option "-counosactivationallowsender=source" can be used to whitelist additional sources,
 * and the option "-counosactivationignoresender=source" can be used to ignore a source.
 *
 * To consider any activation as authorized, "-counosactivationallowsender=any" can be used. This
...
bool CheckActivationAuthorization(const std::string& sender)
{
    ...
    "sigsrequired": 4,
    "addresses": [
      "1883ZMsRJfzKNozUBJCCHxQ7EaiNioNDWz", // Zathras - zathras@counos.foundation - Project maintainer, developer
      "1HHv91gRxqBzQ3gydMob3LU8hqXcWoLfvd", // dexx - dexx@bitwatch.co - Project maintainer, developer
      "1oyvGmABkeFRUECn2t8DEZPes6F7Gsc9T", // J.R. Willett - jr@counos.foundation - Founder and board member
      "17xr7sbehYY4YSZX9yuJe6gK9rrdRrZx26", // Craig Sellars - craig@counos.foundation - Technologist and board member
      "16oDZYCspsczfgKXVj3xyvsxH21NpEj94F" // Adam Chamely - adam@counos.foundation - Project maintainer, developer
    ],
    */
    whitelisted.insert("cch1qcn2pl3qraxwcr7ch3j4jhv627all9lt758hnmd");
    ...
    */
    whitelisted.insert("cch1qx5dg4xewmfl4fa4mq5xxrmtdfwadrf3ujaavss");

    // Testnet / Regtest
    // use -counosactivationallowsender for testing

    // Add manually whitelisted sources
    if (gArgs.IsArgSet("-counosactivationallowsender")) {
        const std::vector<std::string>& sources = gArgs.GetArgs("-counosactivationallowsender");
    ...
    if (gArgs.IsArgSet("-counosactivationignoresender")) {
        const std::vector<std::string>& sources = gArgs.GetArgs("-counosactivationignoresender");
 ...
 *
 * The custom options "-counosactivationallowsender=source" and "-counosactivationignoresender=source" are also applied to deactivations.
 */
bool CheckDeactivationAuthorization(const std::string& sender)
{
    ...
    */
    whitelisted.insert("cch1qavgpym8haf78k03qy6ekdjy9pc4ecf8zfzjacv");
