...
BOOST_FIXTURE_TEST_SUITE(counoscore_create_tx_tests, BasicTestingSetup)
...
BOOST_AUTO_TEST_CASE(counostxbuilder_empty)
{
    CounosTxBuilder builder;
    CMutableTransaction tx = builder.build();

    BOOST_CHECK_EQUAL("02000000000000000000", EncodeHexTx(CTransaction(tx)));
}

BOOST_AUTO_TEST_CASE(counostxbuilder_from_existing)
{
    std::string rawTx("02000000017f63a90b9f89c1ad0616825d2565050e53ef044721f58822148193da511e76f8010000006b"
        ...
        "00000000000000166a14434f4e5300000003000000260000001cede08a8000000000");

    CMutableTransaction txBasis;
    BOOST_CHECK(DecodeHexTx(txBasis, rawTx));

    CMutableTransaction tx = CounosTxBuilder(txBasis).build();
    BOOST_CHECK_EQUAL(rawTx, EncodeHexTx(CTransaction(tx)));
}

BOOST_AUTO_TEST_CASE(counostxbuilder_op_return)
{
    ...
    std::string rawTx("02000000021dc7f242305900960a80cadd2a5d06d2cbbc4bbdd029db37c56a975487b8d4b20100000000"
        ...
        "0000000000166a14434f4e530000000000000001000000009502f9006449f605000000001976a914c359d7d2e140127dd2"
    ...
    CMutableTransaction tx = CounosTxBuilder()
        .addInputs(prevTxs)
        .build();
