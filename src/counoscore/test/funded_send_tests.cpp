    ...
    std::vector<CTxDestination> CreateDestinations(std::vector<CAmount> amounts) {
        ...
        for (auto amount : amounts) {
            ...
            if (amount > 0) {
                ...
                recipients.push_back({GetScriptForDestination(dest), CounosGetDustThreshold(GetScriptForDestination(dest)), false});
            }
        }
        AddTx(recipients);
...
BOOST_FIXTURE_TEST_SUITE(counoscore_funded_send_tests, FundedSendTestingSetup)
