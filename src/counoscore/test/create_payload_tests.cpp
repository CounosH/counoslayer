...
BOOST_FIXTURE_TEST_SUITE(counoscore_create_payload_tests, BasicTestingSetup)
...
BOOST_AUTO_TEST_CASE(payload_send_to_owners)
{
    ...
    std::vector<unsigned char> vch = CreatePayload_SendToOwners(
        static_cast<uint32_t>(1),          // property: COUNOS
        static_cast<int64_t>(100000000),   // amount to transfer: 1.0 COUNOS (in willets)
        static_cast<uint32_t>(1));         // property: COUNOS
...
{
    // Sell tokens for counoshs [type 20, version 1]
    std::vector<unsigned char> vch = CreatePayload_DExSell(
        static_cast<uint32_t>(1),         // property: MSC
        static_cast<int64_t>(100000000),  // amount to transfer: 1.0 MSC (in willets)
        static_cast<int64_t>(20000000),   // amount desired: 0.2 CCH (in satoshis)
...
{
    // Purchase tokens with counoshs [type 22, version 0]
...
BOOST_AUTO_TEST_CASE(payload_create_property)
{
    ...
    std::vector<unsigned char> vch = CreatePayload_IssuanceFixed(
        ...
        std::string("CounosH Mining"),       // subcategory
...
BOOST_AUTO_TEST_CASE(payload_freeze_tokens)
{
    ...
    std::vector<unsigned char> vch = CreatePayload_FreezeTokens(
        ...
        std::string("cch1q6gj3g8v4zekfq9eygwynj5fjr9mukjvaclzgam"));         // reference address
...
{
    // Counos Core feature activation [type 65533, version 65535]
...
{
    // Counos Core feature activation [type 65534, version 65535]
...
BOOST_AUTO_TEST_CASE(payload_counoscore_alert_block)
{
    // Counos Core client notification [type 65535, version 65535]
    std::vector<unsigned char> vch = CreatePayload_CounosCoreAlert(
...
BOOST_AUTO_TEST_CASE(payload_counoscore_alert_blockexpiry)
{
    // Counos Core client notification [type 65535, version 65535]
    std::vector<unsigned char> vch = CreatePayload_CounosCoreAlert(
...
BOOST_AUTO_TEST_CASE(payload_counoscore_alert_minclient)
{
    // Counos Core client notification [type 65535, version 65535]
    std::vector<unsigned char> vch = CreatePayload_CounosCoreAlert(
