...
// Default log files
const std::string LOG_FILENAME    = "counoscore.log";
 ...
 *
 * The log file can be specified via startup option "--counoslogfile=/path/to/counoscore.log",
 * and if none is provided, then the client's datadir is used as default location.
 */
static fs::path GetLogPath()
{
    fs::path pathLogFile;
    std::string strLogPath = gArgs.GetArg("-counoslogfile", "");
...
int LogFilePrint(const std::string& str)
{
    ...
    if (LogInstance().m_print_to_console) {
    ...
    else if (LogInstance().m_print_to_file || fCounosCoreConsoleLog) {
        ...
        if (fReopenCounosCoreLog) {
            fReopenCounosCoreLog = false;
            fs::path pathDebug = GetLogPath();
...
/**
 * Determine whether to override compiled debug levels via enumerating startup option --counosdebug.
 *
 * Example usage (granular categories)    : --counosdebug=parser --counosdebug=metadex1 --counosdebug=ui
 * Example usage (enable all categories)  : --counosdebug=all
 * Example usage (disable all debugging)  : --counosdebug=none
 * Example usage (disable all except XYZ) : --counosdebug=none --counosdebug=parser --counosdebug=sto
 */
void InitDebugLogLevels()
{
    if (!gArgs.IsArgSet("-counosdebug")) {
        return;
    }

    const std::vector<std::string>& debugLevels = gArgs.GetArgs("-counosdebug");
