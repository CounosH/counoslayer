 ...
 */
bool CounosCore_Encode_ClassB(const std::string& senderAddress, const CPubKey& redeemingPubKey,
        const std::vector<unsigned char>& vchPayload, std::vector<std::pair<CScript, int64_t> >& vecOutputs, CAmount *outputAmount)
{
    ...
    while (nRemainingBytes > 0) {
        ...
        if (outputAmount)
            *outputAmount += CounosGetDustThreshold(scriptMultisigOut);
        vecOutputs.push_back(std::make_pair(scriptMultisigOut, CounosGetDustThreshold(scriptMultisigOut)));
    }

    // Add the Exodus marker output
    CScript scriptExodusOutput = GetScriptForDestination(ExodusAddress());
    if (outputAmount)
        *outputAmount += CounosGetDustThreshold(scriptExodusOutput);
    vecOutputs.push_back(std::make_pair(scriptExodusOutput, CounosGetDustThreshold(scriptExodusOutput)));
    return true;
}
 ...
 */
bool CounosCore_Encode_ClassC(const std::vector<unsigned char>& vchPayload,
