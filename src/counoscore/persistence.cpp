...
//! Number of "Dev Counos" of the last processed block (needed to save global state)
...
{
    uint32_t nextSPID = pDbSpInfo->peekNextSPID(COUNOS_PROPERTY_MSC);
    uint32_t nextTestSPID = pDbSpInfo->peekNextSPID(COUNOS_PROPERTY_TMSC);
...
// seller-address, offer_block, amount, property, desired CCH , property_desired, fee, blocktimelimit
...
static int input_mp_offers_string(const std::string& s)
{
    ...
    if (COUNOS_PROPERTY_CCH != prop_desired) return -1;
