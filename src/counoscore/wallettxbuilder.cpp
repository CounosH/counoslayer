        ...
        CAmount minFee)
{
    ...
    if (!UseEncodingClassC(payload.size() + 1 /* OP_RETURN */ + 2 /* pushdata opcodes */)) counosTxClass = COUNOS_CLASS_B;
...
//    PrintToLog("senderAddress: %d\n", senderAddress);
    ...
    switch(counosTxClass) {
        case COUNOS_CLASS_B: { // declaring vars in a switch here so use an explicit code block
            ...
            if (!CounosCore_Encode_ClassB(senderAddress, redeemingPubKey, payload, vecSend, &outputAmount)) { return MP_ENCODING_ERROR; }
        break; }
        case COUNOS_CLASS_C:
            if(!CounosCore_Encode_ClassC(payload,vecSend)) { return MP_ENCODING_ERROR; }
        break;
    }
    ...
    if (!receiverAddresses.empty()) {
        for (const std::string& receiverAddress : receiverAddresses) {
            ...
            if (!scriptPubKey.empty()) {
                outputAmount += 0 < referenceAmount ? referenceAmount : CounosGetDustThreshold(scriptPubKey);
                vecSend.push_back(std::make_pair(scriptPubKey, 0 < referenceAmount ? referenceAmount : CounosGetDustThreshold(scriptPubKey)));
            }
        }
    }
    ...
    while (createTX) {
        ...
        PrintToLog("Increasing fee. selected: %d \n", selected);
        ...
        // Ask the wallet to create the transaction (note mining fee determined by CounosH Core params)
        ...
        NodeContext& node)
{
    ...
    if (!receiverAddress.empty() && receiverAddress != feeAddress) {
        max_outputs = 3;
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(receiverAddress));
        vecSend.push_back(std::make_pair(scriptPubKey, CounosGetDustThreshold(scriptPubKey)));
    }
...
/**
 * Used by the counos_senddexpay RPC call to creates and send a
...
int CreateDExTransaction(interfaces::Wallet* pwallet, const std::string& buyerAddress, const std::string& sellerAddress, const CAmount& nAmount, uint256& txid)
{
    ...
    CAmount dust = CounosGetDustThreshold(exodus);
