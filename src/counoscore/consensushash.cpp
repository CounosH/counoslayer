...
bool ShouldConsensusHashBlock(int block) {
    ...
    if (!gArgs.IsArgSet("-counosshowblockconsensushash")) {
        return false;
    }

    const std::vector<std::string>& vecBlocks = gArgs.GetArgs("-counosshowblockconsensushash");
...
std::string GenerateConsensusString(const CMPOffer& offerObj, const std::string& address)
{
    return strprintf("%s|%s|%d|%d|%d|%d|%d",
            ...
            offerObj.getCCHDesiredOriginal(), offerObj.getMinFee(), offerObj.getBlockTimeLimit());
}
 ...
 *
 * For increased flexibility, so other implementations like CounosWallet and CounosChest can
