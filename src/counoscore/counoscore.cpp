...
#include <counoscore/persistence.h>
...
static std::string exodus_address = "cch1q6gj3g8v4zekfq9eygwynj5fjr9mukjvaclzgam";

//! Mainnet Exodus address
static const std::string exodus_mainnet = "cch1q6gj3g8v4zekfq9eygwynj5fjr9mukjvaclzgam";
//! Testnet Exodus address
static const std::string exodus_testnet = "cch1q5h8klytg2yr6q8wsvfnw5kc035jkh0val52jgu"; // cch1q6gj3g8v4zekfq9eygwynj5fjr9mukjvaclzgam
//! Testnet Exodus crowdsale address
static const std::string getmoney_testnet = "cch1qrhk2ea3ce8uw76alqr69glwng84np2nvde0lq3"; // cch1qrhk2ea3ce8uw76alqr69glwng84np2nvde0lq3
 ...
 *
 * Can be set with configuration "-autocommit" or RPC "setautocommit_COUNOS".
 */
bool autoCommit = true;

//! Number of "Dev Counos" of the last processed block
int64_t exodus_prev = 0;

//! Path for file based persistence
fs::path pathStateFiles;

//! Flag to indicate whether Counos Core was initialized
...
//! LevelDB based storage for storing Counos transaction validation and position in block data
CCounosTransactionDB* mastercore::pDbTransaction;
//! LevelDB based storage for the MetaDEx fee cache
CCounosFeeCache* mastercore::pDbFeeCache;
//! LevelDB based storage for the MetaDEx fee distributions
CCounosFeeHistory* mastercore::pDbFeeHistory;
...
std::string mastercore::strMPProperty(uint32_t propertyId)
{
    ...
    if (0x80000000 & propertyId) {
        ...
        switch (propertyId) {
            case COUNOS_PROPERTY_CCH: str = "CCH";
                break;
            case COUNOS_PROPERTY_MSC: str = "COUN";
                break;
            case COUNOS_PROPERTY_TMSC: str = "TCOUN";
                break;
            default:
...
{
    if ((COUNOS_PROPERTY_TMSC == propertyId) || (TEST_ECO_PROPERTY_1 <= propertyId)) return true;
...
{
    if ((COUNOS_PROPERTY_CCH != propertyId) && !isTestEcosystemProperty(propertyId)) return true;
 ...
 * https://github.com/CounosLayer/spec#development-mastercoins-dev-msc-previously-reward-mastercoins
 *
 * Note:
...
static int64_t calculate_and_update_devmsc(unsigned int nTime, int block)
{
    ...
    if (devmsc > all_reward || 0 > devmsc) {
        PrintToLog("%s(): ERROR: insane number of Dev COUNOS (nTime=%d, exodus_prev=%d, devmsc=%d)\n", __func__, nTime, exodus_prev, devmsc);
        return 0;
    }

    if (exodus_delta > 0) {
        update_tally_map(exodus_address, COUNOS_PROPERTY_MSC, exodus_delta, BALANCE);
        exodus_prev = devmsc;
    }

    NotifyTotalTokensChanged(COUNOS_PROPERTY_MSC, block);
...
void CheckWalletUpdate(bool forceUpdate)
{
    ...
    if (!WalletCacheUpdate()) {
        // no balance changes were detected that affect wallet addresses, signal a generic change to overall Counos state
        if (!forceUpdate) {
    ...
    // balance changes were found in the wallet, update the global totals and signal a Counos balance change
    ...
    // signal an Counos balance change
    uiInterface.CounosBalanceChanged();
#endif
}
...
static bool TXExodusFundraiser(const CTransaction& tx, const std::string& sender, int64_t amountInvested, int nBlock, unsigned int nTime)
{
    ...
    if (nBlock >= params.GENESIS_BLOCK && nBlock <= params.LAST_EXODUS_BLOCK) {
        ...
        if (amountGenerated > 0) {
            ...
            assert(update_tally_map(sender, COUNOS_PROPERTY_MSC, amountGenerated, BALANCE));
            assert(update_tally_map(sender, COUNOS_PROPERTY_TMSC, amountGenerated, BALANCE));
...
//! Cache for potential Counos Layer transactions
...
/**
 * Checks, if transaction has any Counos marker.
 *
 * Note: this may include invalid or malformed Counos Layer transactions!
 *
 * MUST NOT BE USED FOR CONSENSUS CRITICAL STUFF!
 */
static bool HasMarkerUnsafe(const CTransactionRef& tx)
{
    const std::string strClassC("434f4e53");
    const std::string strClassAB("0014d225141d95166c90172443893951321977cb499d");
    const std::string strClassABTest("0014a5cf6f91685107a01dd06266ea5b0f8d256bbd9d");
    const std::string strClassMoney("00141decacf638c9f8ef6bbf00f4547dd341eb30aa6c");
...
int mastercore::GetEncodingClass(const CTransaction& tx, int nBlock)
{
    ...
    /* Fast Search
     * Perform a string comparison on hex for each scriptPubKey & look directly for Exodus hash160 bytes or counos marker bytes
     * This allows to drop non-Counos transactions with less work
     */
    std::string strClassC = "434f4e53";
    std::string strClassAB = "0014d225141d95166c90172443893951321977cb499d";
    ...
    for (unsigned int n = 0; n < tx.vout.size(); ++n) {
        ...
        if (strSPB != strClassAB) { // not an exodus marker
            if (nBlock < 64670) { // class C not enabled yet, no need to search for marker bytes
                continue;
            } else {
    ...
    for (unsigned int n = 0; n < tx.vout.size(); ++n) {
        ...
        if (outType == TX_WITNESS_V0_KEYHASH) {
            CTxDestination dest;
            if (ExtractDestination(output.scriptPubKey, dest)) {
                if (dest == ExodusAddress()) {
                    hasExodus = true;
        ...
        if (outType == TX_NULL_DATA) {
            ...
            // or starts with the "counos" marker
    ...
    if (hasOpReturn) {
        return COUNOS_CLASS_C;
    }
    if (hasExodus && hasMultisig) {
        return COUNOS_CLASS_B;
    }
    if (hasExodus || hasMoney) {
        return COUNOS_CLASS_A;
    }
...
{
    static unsigned int nCacheSize = gArgs.GetArg("-counostxcache", 500000);
...
static int parseTransaction(bool bRPConly, const CTransaction& wtx, int nBlock, unsigned int idx, CMPTransaction& mp_tx, unsigned int nTime, const std::shared_ptr<std::map<COutPoint, Coin>> removedCoins = nullptr)
{
    ...
    int counosClass = GetEncodingClass(wtx, nBlock);

    if (counosClass == NO_MARKER) {
        return -1; // No Exodus/Counos marker, thus not a valid Counos transaction
    }
    ...
    if (counosClass == COUNOS_CLASS_A) {
        ...
        for (unsigned k = 0; k < script_data.size(); ++k) { // Step 1, locate the data packet
            ...
            if ("0000000000000001" == strSub || "0000000000000002" == strSub) { // peek & decode comparison
                ...
                } else { // invalidate - Class A cannot be more than one data packet - possible collision, treat as default (CCH payment)
                    strDataAddress.clear(); //empty strScriptData to block further parsing
                    if (msc_debug_parser_data) PrintToLog("Multiple Data Addresses found (collision?) Class A invalidated, defaulting to CCH payment\n");
                    break;
                }
            }
        }
                        ...
                        for (unsigned int exodus_idx = 0; exodus_idx < ExodusValues.size(); exodus_idx++) {
                            if (value_data[k] == ExodusValues[exodus_idx]) { //this output matches data address value and exodus address value, choose as ref
                                if (strRefAddress.empty()) {
                                    ...
                                    strRefAddress.clear();
                                    if (msc_debug_parser_data) PrintToLog("Reference Address collision, multiple potential candidates. Class A invalidated, defaulting to CCH payment\n");
                                    break;
                                }
                            }
                        }
        ...
        if (strRefAddress.empty()) {
            strDataAddress.clear(); // last validation step, if strRefAddress is empty, blank strDataAddress so we default to CCH payment
        }
        if (!strDataAddress.empty()) { // valid Class A packet almost ready
            ...
            if ((!bRPConly || msc_debug_parser_readonly) && msc_debug_parser_dex) {
                ...
                PrintToLog("!! this may be the CCH payment for an offer !!\n");
            }
        }
    }
    // ### CLASS B / CLASS C PARSING ###
    if ((counosClass == COUNOS_CLASS_B) || (counosClass == COUNOS_CLASS_C)) {
    ...
    mp_tx.Set(strSender, strReference, 0, wtx.GetHash(), nBlock, idx, (unsigned char *)&single_pkt, packet_size, counosClass, (inAll-outAll));

    // TODO: the following is a bit awful
    // Provide a hint for DEx payments
    if (counosClass == COUNOS_CLASS_A && packet_size == 0) {
        return 1;
    }
...
static bool HandleDExPayments(const CTransaction& tx, int nBlock, const std::string& strSender)
{
    ...
    for (unsigned int n = 0; n < tx.vout.size(); ++n) {
        ...
        if (ExtractDestination(tx.vout[n].scriptPubKey, dest)) {
            ...
            // check everything and pay CCH for the property we are buying here...
...
{
    int nTimeBetweenProgressReports = gArgs.GetArg("-counosprogressfrequency", 30);  // seconds
    ...
    bool seedBlockFilterEnabled = gArgs.GetBoolArg("-counosseedblockfilter", true);
...
void RewindDBsAndState(int nHeight, int nBlockPrev = 0, bool fInitialParse = false)
{
...

    {
        ...
        uiInterface.CounosStateInvalidated();
...
/**
 * Global handler to initialize Counos Core.
 *
 * @return An exit code, indicating success or failure
 */
int mastercore_init()
{
...

    {
        ...
        PrintToConsole("Initializing Counos Core v%s [%s]\n", CounosCoreVersion(), Params().NetworkIDString());

        PrintToLog("\nInitializing Counos Core v%s [%s]\n", CounosCoreVersion(), Params().NetworkIDString());
        ...
        if (!gArgs.GetBoolArg("-autocommit", true)) {
            PrintToLog("Process was started with --autocommit set to false. "
                    "Created Counos transactions will not be committed to wallet or broadcast.\n");
            autoCommit = false;
        }
        ...
        if (gArgs.GetBoolArg("-startclean", false)) {
            ...
            try {
                ...
                fs::path counosTXDBPath = GetDataDir() / "Counos_TXDB";
                fs::path feesPath = GetDataDir() / "COUNOS_feecache";
                fs::path feeHistoryPath = GetDataDir() / "COUNOS_feehistory";
                fs::path nftdbPath = GetDataDir() / "COUNOS_nftdb";
                ...
                if (fs::exists(counosTXDBPath)) fs::remove_all(counosTXDBPath);
        ...
        pDbTransaction = new CCounosTransactionDB(GetDataDir() / "Counos_TXDB", fReindex);
        pDbFeeCache = new CCounosFeeCache(GetDataDir() / "COUNOS_feecache", fReindex);
        pDbFeeHistory = new CCounosFeeHistory(GetDataDir() / "COUNOS_feehistory", fReindex);
        pDbNFT = new CMPNonFungibleTokensDB(GetDataDir() / "COUNOS_nftdb", fReindex);
...

    {
        ...
        if (nWaterlineBlock > 0) {
            ...
            if (inconsistentDb) strReason = "INCONSISTENT DB DETECTED!\n"
                    ...
                    "ALL COUNOS LAYER TRANSACTIONS TO AVOID INCONSISTENCIES!\n"
                    "\n!!! WARNING !!!";
        ...
        if (inconsistentDb) {
            std::string strAlert("INCONSISTENT DB DETECTED! IF YOU ARE USING AN OVERLAY DB, YOU MAY NEED TO REPROCESS"
                    "ALL COUNOS LAYER TRANSACTIONS TO AVOID INCONSISTENCIES!");
            AddAlert("counoscore", ALERT_CLIENT_VERSION_EXPIRY, std::numeric_limits<uint32_t>::max(), strAlert);
            DoWarning(strAlert);
        }
        ...
        if (msc_debug_exo) {
            int64_t exodus_balance = GetTokenBalance(exodus_address, COUNOS_PROPERTY_MSC, BALANCE);
    ...
    PrintToConsole("Counos Core initialization completed\n");

    return 0;
}

/**
 * Global handler to shut down Counos Core.
 *
...
int mastercore_shutdown()
{
    ...
    PrintToLog("\nCounos Core shutdown completed\n");
    PrintToLog("Shutdown time: %s\n", FormatISO8601DateTime(GetTime()));

    PrintToConsole("Counos Core shutdown completed\n");
 ...
 *
 * @return True, if the transaction was an Exodus purchase, DEx payment or a valid Counos transaction
 */
bool mastercore_handler_tx(const CTransaction& tx, int nBlock, unsigned int idx, const CBlockIndex* pBlockIndex, const std::shared_ptr<std::map<COutPoint, Coin> > removedCoins)
{
...

    {
        ...
        if (pop_ret > 0) {
            assert(mp_obj.getEncodingClass() == COUNOS_CLASS_A);
...
{
    size_t nTotalSize = nDataSize + GetOmMarker().size(); // Marker "counos"
        ...
        unsigned int countMP)
{
    ...
    bool checkpointValid;
    {
        ...
        //    paying CCH for the offer in several installments)
        ...
        if (msc_debug_exo) {
            int64_t balance = GetTokenBalance(exodus_address, COUNOS_PROPERTY_MSC, BALANCE);
 ...
 *
 * Main network:
 *   cch1q6gj3g8v4zekfq9eygwynj5fjr9mukjvaclzgam
 *
 * Test network:
 *   cch1q5h8klytg2yr6q8wsvfnw5kc035jkh0val52jgu
 *
 ...
 *
 * Test network:
 *   cch1q5h8klytg2yr6q8wsvfnw5kc035jkh0val52jgu (for blocks <  270775)
 *   cch1qrhk2ea3ce8uw76alqr69glwng84np2nvde0lq3 (for blocks >= 270775)
 *
...
{ //0x6f, 0x6d, 0x6e, 0x69     0x63, 0x6f, 0x75, 0x6e, 0x6f,0x73     0x43, 0x4f, 0x4e, 0x53
    static unsigned char pch[] = {0x43, 0x4f, 0x4e, 0x53}; // Hex-encoded: "counos"
