...
TxBuilder& TxBuilder::addChange(const CTxDestination& destination, const CCoinsViewCache& view, int64_t txFee, uint32_t position)
{
    ...
    int64_t minValue = CounosGetDustThreshold(scriptPubKey);
...
CounosTxBuilder::CounosTxBuilder()
  : TxBuilder()
{
}

/** Creates a new Counos transaction builder to extend a transaction. */
CounosTxBuilder::CounosTxBuilder(const CMutableTransaction& transactionIn)
  : TxBuilder(transactionIn)
{
}

/** Adds a collection of previous outputs as inputs to the transaction. */
CounosTxBuilder& CounosTxBuilder::addInputs(const std::vector<PrevTxsEntry>& prevTxs)
{
...
CounosTxBuilder& CounosTxBuilder::addReference(const std::string& destination, int64_t value)
{
    ...
    return (CounosTxBuilder&) TxBuilder::addOutput(scriptPubKey, value);
}

/** Embeds a payload with class C (op-return) encoding. */
CounosTxBuilder& CounosTxBuilder::addOpReturn(const std::vector<unsigned char>& data)
{
    std::vector<std::pair<CScript, int64_t> > outputs;

    if (!CounosCore_Encode_ClassC(data, outputs)) {
        return *this;
    }

    return (CounosTxBuilder&) TxBuilder::addOutputs(outputs);
}

/** Embeds a payload with class B (bare-multisig) encoding. */
CounosTxBuilder& CounosTxBuilder::addMultisig(const std::vector<unsigned char>& data, const std::string& seed, const CPubKey& pubKey)
{
    std::vector<std::pair<CScript, int64_t> > outputs;

    if (!CounosCore_Encode_ClassB(seed, pubKey, data, outputs)) {
        return *this;
    }
...
/** Adds an output for change. */
CounosTxBuilder& CounosTxBuilder::addChange(const std::string& destination, const CCoinsViewCache& view, int64_t txFee, uint32_t position)
{
    CTxDestination dest = DecodeDestination(destination);

    return (CounosTxBuilder&) TxBuilder::addChange(dest, view, txFee, position);
}
